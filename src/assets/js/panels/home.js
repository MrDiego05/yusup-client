 ...
 */
import { config, database, logger, changePanel, appdata, setStatus, pkg, popup, ModpackSync, NeoForgeSync } from '../utils.js'
...
class Home {
        ...
        if (news) {
            ...
            blockNews.innerHTML = `
                <div class="news-header">
                    ...
                    <div class="news-content">
                        <div class="bbWrapper">
                            <p>No se pudo contactar con el servidor de noticias.</br>Por favor, comprueba tu configuración.</p>
                        </div>
                    </div>`
            newsElement.appendChild(blockNews);
        }
    ...
    async startGame() {
        ...
        if (options.modpack_url) {
            try {
                infoStarting.innerHTML = `Iniciando sincronización de Modpack...`;
        ...
        if (options.loader.loader_type === 'neoforge') {
            try {
                infoStarting.innerHTML = `Instalando NeoForge...`;
                const javaPath = configClient.java_config.java_path || 'java'; // Use system java if not set
                const neoForgeSync = new NeoForgeSync(gamePath, javaPath);
                
                const versionName = await neoForgeSync.install(
                    options.loader.minecraft_version, 
                    options.loader.loader_version, 
                ...
                // Override launch options for NeoForge
                opt.version = versionName;
                opt.loader.enable = false; // Bypass default forge/fabric installer in minecraft-java-core
            } catch (err) {
                let popupError = new popup()
                popupError.openPopup({
        ...
        launch.on('data', (e) => {
            ...
            ipcRenderer.send('main-window-progress-load')
            infoStarting.innerHTML = `Iniciando juego...`
            console.log(e);
        })
